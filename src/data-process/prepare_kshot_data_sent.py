...
# Similar with the original prepare_kshot_data.py, but adapted for LAP14. For REST15 and REST16 we split based on category (to make sure we cover every category)
# but LAP14 does not have categories (or are too fine-grained, rendering them not that usefull), so we split based on sentiment
# NOTE: This script could have been more generic, allowing for a check of on what to split
def main():
    parser = get_parser()
    config = parser.parse_args()
    all_sents_original, all_labels_original, unique_labels_original = read_absa_quad_from_file(config.input_file)
    # print(unique_labels_original)
    unique_labels_original = Counter([y[2] for x in all_labels_original for y in x]) 
    print(unique_labels_original)
...
# python3 src/data/laptop_data_conversion.py \
#     --train_file data/14lap/train.txt \
#     --dev_file data/14lap/dev.txt \
#     --test_file data/14lap/test.txt \
#     --output_dir data/lap14

# python3 src/data/prepare_kshot_data_sent.py  \
#     --input_file data/lap14/train.txt \
#     --output_dir fdata/lap14/5 \
#     --num_shot 5 \
#     --num_repeat 1