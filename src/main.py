...
def init_args():
    ...
    parser.add_argument("--data_cou", default='5', type=str, required=True,
                        help="数据量")
...
def get_dataset(tokenizer, type_path, args):
    return ABSADataset(tokenizer=tokenizer, data_dir=args.dataset, absa_task = args.task, data_count=args.data_cou,
...
def evaluate(data_loader, model, reviews, sents):
    """
...
dataset = ABSADataset(tokenizer=tokenizer, data_dir=args.dataset, absa_task = args.task, data_count=args.data_cou, 
...
dataset_f = ABSADataset(tokenizer=tokenizer, data_dir=args.dataset, absa_task = args.task, data_count=args.data_cou, 
                      data_type='dev', max_len=args.max_seq_length)
data_sample_f = dataset_f[7]  # a random data sample
print('Input :', tokenizer.decode(data_sample_f['source_ids'], skip_special_tokens=True))
print('Output:', tokenizer.decode(data_sample_f['target_ids'], skip_special_tokens=True))
...
if args.do_direct_eval:
    ...
    scores = evaluate(test_loader, model, reviews, sents)
